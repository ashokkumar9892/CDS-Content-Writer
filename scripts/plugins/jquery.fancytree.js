...
function _makeNodeTitleMatcher(s){
	...
	return function(node){
		return node.name.toLowerCase().indexOf(s) >= 0;
	};
}
...
var i,
	...
	NODE_ATTRS = "expanded extraClasses folder hideCheckbox key lazy refKey selected name tooltip unselectable".split(" "),
	NODE_ATTR_MAP = {},
...
FancytreeNode.prototype = /** @lends FancytreeNode# */{
	...
	// TODO: expand(flag)
	/**Find all nodes that contain `match` in the name.
	 *
	...
	/**Find first node that contains `match` in the name (not including self).
	 *
	 * @param {string | function(node)} match string to search for, of a function that
	 * returns `true` if a node is matched.
...
//		return this.parent.children.indexOf(this);
	 ...
	 */
	isLoaded: function() {
	    return this.data.isOpen || this.hasChildren() !== undefined; // Also checks if the only child is a status node
	},
	...
	moveTo: function(targetNode, mode, map) {
		...
		if( this.parent.children.length === 1 ) {
		    this.parent.children = !this.parent.data.isOpen ? [] : null;
	...
	navigate: function(where, activate) {
		...
		switch( where ) {
			...
			case KC.LEFT:
				...
				break;
			case KC.RIGHT:
			    if (!this.expanded && (this.children || !this.data.isOpen)) {
					this.setExpanded();
					_goto(this);
...
//        this.tree.debug("setTimeout(%s, %s): %s", mode, ms, this.tree.timer);
	...
	scrollIntoView: function(effects, options) {
		...
		// Gokul - problems with docking panel_assert($(this.span).is(":visible"), "scrollIntoView node is invisible"); // otherwise we cannot calc offsets
...
//                dict.children = null;
	...
	toString: function() {
		return "<FancytreeNode(#" + this.key + ", '" + this.name + "')>";
	},
...
//            window.console.log("getNodeByKey(" + key + "): ", node.key);
    ...
    getNodeByStorableId: function(id) {
    ...
    getNodeByName: function (name) {
        ...
        root.visit(function (node) {
            try {
                ...
                console.log('getNodeByName: node=' + JSON.stringify(node));
                console.log('getNodeByName: name=' + name);
                console.log('getNodeByName: error=' + JSON.stringify(e));
            }
        }, true);
        return match;
    },
...
//                node = _findDirectChild(root, key);
				...
				if(!node){
					...
					break;
				} else if (node.data.isOpen || (node.hasChildren() !== undefined)) {
...
//		this.debug("_trigger(" + type + "): '" + ctx.node.name + "'", ctx);
...
//				this._callHook("nodeSetExpanded", ctx, true);
	...
	nodeDblclick: function(ctx) {
		// TODO: return promise?
		if( ctx.targetType === "name" && ctx.options.clickFolderMode === 4) {
//			this.nodeSetFocus(ctx);
//			this._callHook("nodeSetActive", ctx, true);
			this._callHook("nodeToggleExpanded", ctx);
		}
		// TODO: prevent text selection on dblclicks
		if( ctx.targetType === "name" ) {
...
//				alert("remove child markup for " + node);
	 ...
	 */
	nodeRenderTitle: function(ctx, name) {
		...
		switch (node.data.type) {
		    case 1: title = "Knowledge Module"; break;
		    case 2:
		        {
		            if (node.data.isAtp) {
		                if (node.name.indexOf("(ATP)") < 0) {
		                    node.name = node.name + ' (ATP)';
		                }
		            }
		            title = "Value Set";
		        } break;
		    case 3: title = "Patient Record"; break;
		    case 4: title = "Test Case"; break;
		    case 7: title = "Content Program"; break;
		    case 100: title = "Building Block"; break;
		}
		if (icon && typeof icon === "string") {
		    imageSrc = (icon.charAt(0) === "/") ? icon : ((opts.imagePath || "") + icon);
			ares.push("<img src='" + imageSrc + "' class='fancytree-icon' alt='' title='" + title + "' />");
		} else if ( node.data.iconclass ) {
			// TODO: review and test and document
			ares.push("<span " + role + " class='fancytree-custom-icon" + " " + node.data.iconclass +  "' title='" + title + "'></span>");
			...
			ares.push("<span " + role + " class='fancytree-icon' title='" + title + "'></span>");
		}
		...
		if(!nodeTitle){
			tooltip = node.tooltip ? " name='" + FT.escapeHtml(node.tooltip) + "'" : "";
			id = aria ? " id='ftal_" + node.key + "'" : "";
			role = aria ? " role='treeitem'" : "";
			tabindex = opts.titlesTabbable ? " tabindex='0'" : "";

			nodeTitle = "<span " + role + " class='fancytree-title'" + id + tooltip + tabindex + ">" + node.name + "</span>";
			if (node.data.owner && node.data.owner.isPublisher && node.data.isHome) {			  
			    nodeTitle = "<span class='fancytree-admin' title='Administrator'>* </span>"  +nodeTitle ;
			}
		}
		ares.push(nodeTitle);
...
//				.removeAttr("tabindex");
		...
		if (node.data.isFolderType) {
		    if (node.data.isCurrentUsersHome) {
		        cnList.push(cn.homefolder);
		    } else{
		        cnList.push(cn.folder);
		    }
		} else {
		    switch (node.data.type) {
		        case 1: cnList.push(cn.knowledgeModule); break;
		        case 2: cnList.push(cn.valueSet); break;
		        case 3: cnList.push(cn.patientRecord); break;
		        case 4: cnList.push(cn.testCase); break;
		        case 7: cnList.push(cn.contentProgram); break;
		        case 9: cnList.push(cn.logicFragment); break;
		        case 100: cnList.push(cn.buildingBlock); break;
		        case 110: cnList.push(cn.metric); break;
		    }
        }
		...
		if (!node.data.isOpen && node.children == null) {
			cnList.push(cn.lazy);
		}
		if( node.partsel ){
		...
		if( hasChildren === false ){
			...
			cnList.push(cn.combinedExpanderPrefix +
					(node.expanded ? "e" : "c") +
					(!node.data.isOpen && node.children == null ? "d" : "") +
					(isLastSib ? "l" : "")
					);
		}
		cnList.push(cn.combinedIconPrefix +
				(node.expanded ? "e" : "c") +
				(node.data.isFolderType ? "f" : "")
				);
...
//			userEvent = !!ctx.originalEvent,
	...
	nodeSetExpanded: function(ctx, flag, callOpts) {
		...
		if((node.expanded && flag) || (!node.expanded && !flag)){
			...
			return _getResolvedPromise(node);
		} else if (flag && node.data.isOpen && !node.hasChildren()) {
		...
		if (flag && !node.data.isOpen && node.hasChildren() === undefined) {
...
//			this._fixSelectionState(node);
	...
	nodeSetStatus: function(ctx, status, message, details) {
		...
		case "loading":
			...
			if( !node.parent ) {
				_setStatusNode({
					name: tree.options.strings.loading + (message ? " (" + message + ") " : ""),
					tooltip: details,
	...
	//            $().addClass("ui-helper-hidden");
				...
				if(source.children){
					if(source.name){tree.name = source.name;}
...
//      autoFocus: false,
		...
		_classNames: {
			...
			homefolder: "fancytree-home",
			valueSet: "fancytree-vs",
			knowledgeModule: "fancytree-km",
			patientRecord: "fancytree-pr",
			testCase: "fancytree-tc",
			contentProgram: "fancytree-cp",
			logicFragment: "fancytree-lf",
			buildingBlock: "fancytree-bb",
            metric: "fancytree-metric"
		},
		// events
		lazyLoad: null,
		postProcess: null
...
//                 $.error(e);
    ...
    _findById: function(parent, id) {
        ...
        for (i = 0; i < parent.children.length; i++) {
            ...
            if (child['children'] !== undefined) {
                var found = this._findById(child, id);
                if (found) {
                    return found;
                }
            }
    ...
    getNodeByStorableId: function(id) {
        return this.tree.getNodeByStorableId(id);
    }
});
	...
	parseHtml: function($ul) {
		...
		$children.each(function() {
			...
			if( $liSpan.length ) {
				...
				d.tooltip = $liA.attr("name");

			} else {
				// If only a <li> tag is specified, use the trimmed string up to
				// the next child <ul> tag.
				d.name = $li.html();
				iPos = d.name.search(/<ul/i);
				if( iPos >= 0 ){
					d.name = d.name.substring(0, iPos);
				}
			}
			d.name = $.trim(d.name);
			...
			// Parse node options from ID, name and class attributes
			tmp = $li.attr("name");
			if( tmp ){
				d.tooltip = tmp; // overrides <a name='...'>
			}
			tmp = $li.attr("id");
			if( tmp ){
				d.key = tmp;
			...
			if( $ul.length ) {
			    ...
			    d.children = !d.data.isOpen ? undefined : null;
			}
			children.push(d);
